...
residdiag3.nlme = function(fit, limit,plotid=NULL) {
  ...
  #### Extraccion objetos de funcion lme ####
  ...
  #### Extraer informacion del modelo ajustado con lme y el banco de datos ####
  ...
  #### Construccion de las matrices Z y Gam ####
  if (n.levels > 1) { 
    lZi <- list()
    lgi <- list()
    numrow <- numeric()
  ...
  #### Estimacion de la matriz de covarianza de errores conndicionales (modelo condicional de independencia homocedastica) #####
  if (n.levels > 1) {   
    if (!inherits(fit, "lme")) 
  ...
  #### Varianza de residuos marginales ####
  var.resm <- V - X %*% solve(t(X) %*% iV %*% X) %*% t(X) 
  
  #### Residuos marginales estandarizados ####
  ...
  #### Residuos condicionales estandarizados ####
  ...
  #### Residuales minimamente confundidos ####
...
\n")
    ...
    if (plotid == 2)
    {
      par(mfrow = c(1,1))#, mar = c(10, 5, 1, 2))
    ...
    if (plotid == 6)
    {
      par(mfrow = c(1,1))#, mar = c(11, 5, 3, 2))
